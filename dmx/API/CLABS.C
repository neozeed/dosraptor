/*===========================================================================
* CLABS.C - Logic for Creative Labs Sound Boards SB/SBPro/SB 16
*----------------------------------------------------------------------------
...
*----------------------------------------------------------------------------
* $Log:   F:/DMX/API/VCS/CLABS.C_V  $
*  
*     Rev 1.6   29 Oct 1993 01:11:14   pjr
*  Got rid of bogus dnr scaling and added support for 
*  new mix engines.
*  
*     Rev 1.5   11 Oct 1993 06:16:00   pjr
*  Changed DMA size from 8192 to 4096 this allows DMA to be aquired under OS/2
*  
*  
*     Rev 1.4   11 Oct 1993 05:47:48   pjr
*  Rewrote card detection AGAIN!  This time it was verified with a 
*  66Mhz 486 VLB with PCNFS loaded.  It worked with ALL sound blaster
*  compatible cards, and no lockups!
*  
*  Also fixed problem with start of DMA on play mode, SB Pro now works
*  on IRQ-10.
*  
*  
*  
*     Rev 1.3   07 Oct 1993 13:51:50   pjr
*  Removed DEAD code
*  
*  
*     Rev 1.2   07 Oct 1993 13:41:38   pjr
*  Re-hacked SB_Detect to allow selective scanning.
*  
*  
*     Rev 1.1   02 Oct 1993 15:13:10   pjr
*  Revised Autodetect 
*  
*     Rev 1.0   23 Sep 1993 08:23:08   pjr
*  Initial revision.
*---------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
...
// DSP Commands
#define CMD_DSP_ADCRecSample		0x24
#define CMD_DSP_StdPlaySample       0x14
#define CMD_DSP_AIPlaySample        0x1C
#define CMD_DSP_HsPlaySample        0x90
#define CMD_DSP_SetTimeConstant		0x40
#define CMD_DSP_DACSamplingRate     0x41
#define CMD_DSP_ADCSamplingRate     0x42
#define CMD_DSP_SetBlockSize        0x48
#define CMD_DSP_DMA_OFF				0xD0
#define CMD_DSP_DACSpeakerOn		0xD1
#define CMD_DSP_DACSpeakerOff		0xD3
#define CMD_DSP_CMD_DMA_ON			0xD4
#define CMD_DSP_DAC8bit             0xC6
#define CMD_DSP_Voice16ExitAI       0xD9
#define CMD_DSP_Voice08ExitAI       0xDA
#define CMD_DSP_GetVersion			0xE1
#define CMD_DSP_GenerateInt			0xF2
#define CMD_DSP_DetectJazz          0xFA
#define MODE_DSP_StereoUnsigned     0x20

#define DSP_POLL_MAX_TRIES      200
#define REG_MIXER_IntrStatus	0x82
#define BIT_INTRSTATUS_Voice08  0x01
#define BIT_INTRSTATUS_Voice16	0x02
...
typedef struct
{
	WORD	wBaseAddr;				//   0x220
    WORD    wWr_MixerAddx;          //   0x224
    WORD    wRdWr_MixerData;        //   0x225
    WORD    wRdWr_Reset;            //   0x226
    WORD    wRd_Data;               //   0x22A
    WORD    wWr_Data;               //   0x22C
    WORD    wRd_OkToRead;           //   0x22E
    WORD    wRd_AckVoice08Intr;     //   0x22E
    WORD    wRd_AckVoice16Intr;     //   0x22F
} DSP_PORTS;
...
LOCAL int		gIrqDetected;

LOCAL DSP_PORTS	gDspPorts;
LOCAL HWSETUP	gHwSetup;
LOCAL BYTE		*gDmaBuffer;
LOCAL BYTE		*gDmaHandle;
LOCAL WORD		gVersion;
LOCAL BYTE      gFilterStatus;
LOCAL BOOL      gfDmaActive;
LOCAL VOID      ( *SetupDMA )( VOID ) = NULL;
LOCAL WORD 		cAddr[ 6 ] = { 0x210, 0x220, 0x230, 0x240, 0x250, 0x260 };
LOCAL int		cDma[ 3 ] = { 1, 3, 0 };
LOCAL BOOL		gfManualOverride = FALSE;

PRIVATE BOOL clWrite ( BYTE );
