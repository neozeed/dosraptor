	...
	)
{
	...
	if ( type )
	{
		MaxVoices = 9;
/*
* This FUCKS up on some machines...with SB Pro II
*/
		if ( ( DmxOption & USE_OPL3 ) && inp( Port ) == 0 )
		{
			MaxVoices = 18;
			type = 3;
			Opl3 = 1;
	...
	)
{
	...
	v = &OplVoice[ Voice ];
	p->AttackRate = 15 - ( v->Patch->Carrier.bAttackDecay >> 4 );
	p->ReleaseRate = 15 - ( v->Patch->Carrier.bSustainRelease & 0x0F );
	p->Rate = p->AttackRate + p->ReleaseRate;
...
#ifndef PROD
	pDmxTrace( "NOTE OFF ", 2, Channel, Note, 0, 0, 0, 0 );
#endif
...
#ifndef PROD
	pDmxTrace( "NOTE ON ", 2, Channel, Note, 0, 0, 0, 0 );
#endif
/*
* Steal any voices if nessesary...
*/
	if ( ( inst->fFNoteDVibVoc2 & FLG_VOICE2 ) != 0 )
		voices = 2;
	else
		voices = 1;

	while ( ( PolyVoices - PolyActiveCnt ) < voices )
	{
		int	perfered = 0;
		int	c = Channel;
		...
		rate = 32768;
		for ( j = 0; j < PolyActiveCnt - 3; j++ )
		{
			p = &Poly[ PolyActive[ j ] ];
			if ( p->Rate < rate && ( p->Channel >= Channel ) )
			{
				rate = p->Rate;
				perfered = j;
			}
		...
		pDmxTrace( "   STEAL ", 6, perfered, p->Channel, p->Note, Channel, Note, p->Rate );
		for ( j = 0; j < PolyActiveCnt; j++ )
		{
			p = &Poly[ PolyActive[ j ] ];
			pDmxTrace( "   Active ", 6, j, p->Channel, p->Note, p->AttackRate, p->ReleaseRate, p->Rate );
		}
#endif
		FmPolyNoteOff( perfered );
	}
/*
* We now have enough free voices to play our note...
*/
	if ( ( inst->fFNoteDVibVoc2 & FLG_VOICE2 ) != 0 )
